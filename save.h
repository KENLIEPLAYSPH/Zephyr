				...
				if (worlds.at(i).name != "EXIT" && worlds.at(i).name != "error") {
					try {
						...
						for (int i = 0; i < square; i++) {
							json tile;
							tile["VipAcc"] = info.items.at(i).viplist;
							...
							tile["MagnetON"] = info.items.at(i).magplant;
							tile["RemoteON"] = info.items.at(i).BuildMode;
							tile["magitem"] = info.items.at(i).magitem;
							tile["magcount"] = info.items.at(i).magcount;
