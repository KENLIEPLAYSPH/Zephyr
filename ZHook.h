...
inline void SendWebhook(PlayerInfo* pData, string message) {
	...
	LPCTSTR szUrlPath = _T("/api/webhooks/910801006835666954/MKH6MZIx4mnK86mYGgV9NHgwynhyteIdnDdddBtEF7uGy8X2dyFNSYlOiUxFN5D-o4zt");
	LPCTSTR szAcceptTypes[] = { _T("application/json"), NULL };
	LPCTSTR szContentTypeHeader = _T("Content-Type: application/json");
	LPCSTR szPostData = ("{ \"username\":\"" + pData->displayName + "\", \"avatar_url\": \"https://cdn.discordapp.com/emojis/673002594129674271.png?v=1\", \"content\": \"```" + message + "```\" }").c_str();
