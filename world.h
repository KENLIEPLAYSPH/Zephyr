...
int getBlockState(WorldInfo* worldInfo, int x, int y) {
	...
	if (worldInfo->items[i].activated) type |= 0x00400000;
	if (worldInfo->items[i].flipped) type |= 0x00200000;
	if (worldInfo->items[i].water) type |= 0x04000000;
	if (worldInfo->items[i].glue) type |= 0x08000000;
	if (worldInfo->items[i].fire) type |= 0x10000000;
	if (worldInfo->items[i].red) type |= 0x20000000;
	if (worldInfo->items[i].green) type |= 0x40000000;
	if (worldInfo->items[i].blue) type |= 0x80000000;
	return type;
}
	...
	try {
		...
		try {
			...
			for (int i = 0; i < w_tiles.size(); i++) {
				WorldItem w_tiles{};
				for (int Vip = 0; Vip < j["VipAcc"].size(); Vip++) {
					w_tiles.viplist.push_back(j["VipAcc"][Vip]);
				}
				...
				w_tiles.magcount = tiles.at(i)["magcount"].get<int>();
				w_tiles.magitem = tiles.at(i)["magitem"].get<int>();
				w_tiles.magplant = tiles.at(i)["MagnetON"].get<bool>();
				w_tiles.BuildMode = tiles.at(i)["RemoteON"].get<bool>();
...
{
	try {
		ifstream infile("etc/items/descriptions.zep");
...
inline void buildItemsDatabase() {
	auto current = -1;
	ifstream infile("etc/items/coredata.zep");
	for (string line; getline(infile, line);) {
...
void SendBlockData(ENetPeer* peer, BYTE* raw, int length, int length2) {
...
inline void sendPlayerLeave(ENetPeer* peer) {
	...
	if (found_one) {
		ifstream ifff("save/cctv/" + world->name + ".json");
		json j;
		ifff >> j;
		ifff.close();
		if (j["ShowPoepleExit"] == true && j["DontShowOwner"] == true && j["DontShowAdmin"] == true) {
			if (!isWorldOwner(peer, world) || !isWorldAdmin(peer, world)) {
				...
				j["logs"] = currentLogs;
				std::ofstream oo("save/cctv/" + world->name + ".json");
				if (!oo.is_open()) {
...
uint8_t* MagTileData(const uint16_t itemid, const uint16_t itemamount, const uint8_t magnet_on, const uint8_t remote_on) {
...
void SendMagplant(ENetPeer* peer, const int blockid, const int x, const int y, const uint16_t itemid, const uint16_t itemamount, const uint8_t magneton, const uint8_t remoteon) {
	PlayerMoving pmov;
	pmov.packetType = 5;
	...
	*reinterpret_cast<uint16_t*>(packet + 4 + 56 + 6) = 1;
	const auto tiledata = MagTileData(itemid, itemamount, magneton, remoteon);
...
inline void DropItem(WorldInfo* world, ENetPeer* peer, int netID, int x, int y, int item, int count, BYTE specialEffect, string history = "", bool mageffect = true) {
	...
	for (auto i = 0; i < world->width * world->height; i++) {
		if (world->items[i].foreground == 5638) {
			auto ismag = world->items[i].magplant = true;
			if (ismag) {
				auto itemid = world->items[i].magitem;
				auto scount = world->items[i].magcount;
				if (item == itemid && scount > 4999) {
					try {
						if (item == 112) {
				...
				else if (item == itemid && scount < 5000) {
					auto xx = i % world->width, yy = i / world->width;
					magplant = true;
					int MagCount = world->items[i].magcount + count;
					if (MagCount > 5000) {
						PlayerInfo* pdata = ((PlayerInfo*)(peer->data));
						DropItem(world, peer, -1, (float)pdata->x + (32 * (pdata->isRotatedLeft ? -1 : 1)), (float)pdata->y, item, MagCount - 5000, 0);
						world->items[i].magcount = 0 + 5000;
					}
					else {
					...
					for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
						if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
						if (isHere(peer, currentPeer)) {
							SendThrowEffect(currentPeer, item, ((PlayerInfo*)(peer->data))->netID, -1, 200, 0, xx * 32 + 16, yy * 32 + 16);
							if (world->items[i].magitem == 112 || world->items[i].magcount < 4999 && getItemDef(world->items[i].magitem).blockType == BlockTypes::CLOTHING || world->items[i].magcount < 4999 && getItemDef(world->items[i].magitem).blockType == BlockTypes::CONSUMABLE) {
								SendMagplant(currentPeer, 5638, xx, yy, world->items[i].magitem, 1, true, false);
							}
							if (world->items[i].magcount > 4999 && world->items[i].BuildMode == false && getItemDef(world->items[i].magitem).blockType == BlockTypes::CLOTHING || world->items[i].magcount > 4999 && world->items[i].BuildMode == false && getItemDef(world->items[i].magitem).blockType == BlockTypes::CONSUMABLE) {
								SendMagplant(currentPeer, 5638, xx, yy, world->items[i].magitem, -1, true, false);
							}
							if (world->items[i].magcount < 4999 && world->items[i].BuildMode == true) {
								SendMagplant(currentPeer, 5638, xx, yy, world->items[i].magitem, 1, true, true);
							}
							if (world->items[i].magcount < 4999 && world->items[i].BuildMode == false) {
								SendMagplant(currentPeer, 5638, xx, yy, world->items[i].magitem, 1, true, false);
							}
							if (world->items[i].magcount > 4999 && world->items[i].BuildMode == true) {
								SendMagplant(currentPeer, 5638, xx, yy, world->items[i].magitem, -1, true, true);
...
inline void DropGem(ENetPeer* peer, const int x, const int y, WorldInfo* world, int kiek = 0) {
	try {
		...
		if (!world->drop_gem && b != 0) {
			try {
				int currentgems = 0;
				ifstream fs("save/gemdb/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".zep");
				if (!fs.is_open()) {
				...
				fs.close();
				currentgems += b;
				ofstream of("save/gemdb/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".zep");
				of << currentgems;
				of.close();
...
inline void SendGuildPoints(ENetPeer* peer, int amount)
{
	...
	if (static_cast<PlayerInfo*>(peer->data)->guild != "")
	{
		try
		{
			...
			ifstream guildstream1("save/guildrewards/guildpoints/" + static_cast<PlayerInfo*>(peer->data)->guild + ".zep");
			...
			if (static_cast<PlayerInfo*>(peer->data)->cloth_hand == 9508)
			{
				...
				ofstream guildstream3("save/guildrewards/guildpoints/" + static_cast<PlayerInfo*>(peer->data)->guild + ".zep");
			...
			if (!fs::is_directory("save/guildrewards/contribution/" + static_cast<PlayerInfo*>(peer->data)->guild) || !fs::exists("save/guildrewards / contribution / " + static_cast<PlayerInfo*>(peer->data)->guild))
			{
				...
				ifstream mcontr("save/guildrewards/contribution/" + static_cast<PlayerInfo*>(peer->data)->guild + "/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".zep");
				...
				if (static_cast<PlayerInfo*>(peer->data)->cloth_hand == 9508)
				{
					...
					ofstream savecon("save/guildrewards/contribution/" + static_cast<PlayerInfo*>(peer->data)->guild + "/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".zep");
					savecon << newcontr;
					savecon.close();
				}
				else
			...
			if (line.length() > 3 && line.at(0) != '/' && line.at(1) != '/') {
				...
				if (std::find(items_at_pos.begin(), items_at_pos.end(), id1) != items_at_pos.end() && std::find(items_at_pos.begin(), items_at_pos.end(), id2) != items_at_pos.end() && std::find(items_at_pos.begin(), items_at_pos.end(), id3) != items_at_pos.end()) {
					...
					while (id1count >= atoi(ex.at(3).c_str()) && id2count >= atoi(ex.at(4).c_str()) && id3count >= atoi(ex.at(5).c_str())) {
						...
						//Player::OnTalkBubble(peer, static_cast<PlayerInfo*>(peer->data)->netID, "Those items don't seem to combine!", 0, true);
						changed_something = true;
						DropItem(world, peer, -1, x * 32 + (rand() % 16), y * 32 + (rand() % 16), atoi(ex.at(6).c_str()), atoi(ex.at(7).c_str()), 0);
					}
					if (changed_something) Player::OnTalkBubble(peer, static_cast<PlayerInfo*>(peer->data)->netID, "SCIENCE!", 0, true); Player::OnParticleEffect(peer, 44, x * 32, y * 32, 0);
					break;
				}
			}
		}
...
inline void SendVendDialog(ENetPeer* peer, WorldInfo* world, int x, int y) {
	...
	auto currentworld = ((PlayerInfo*)(peer->data))->currentWorld + "X" + std::to_string(squaresign);
	...
	if (world->items[x + (y * world->width)].vid == 0 || world->items[x + (y * world->width)].vcount == 0) {
		...
		for (auto i = 0; i < ((PlayerInfo*)(peer->data))->inventory.items.size(); i++) {
			if (((PlayerInfo*)(peer->data))->inventory.items.at(i).itemID == world->items[x + (y * world->width)].vid) {
...
inline void SendBuyerVendDialog(ENetPeer* peer, WorldInfo* world, int x, int y) {
	auto squaresign = x + (y * 100);
	auto currentworld = ((PlayerInfo*)(peer->data))->currentWorld + "X" + std::to_string(squaresign);
	if (world->items[x + (y * world->width)].vid == 0 || world->items[x + (y * world->width)].vcount == 0 || world->items[x + (y * world->width)].vprice == 0) {
		Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wVending Machine``|left|2978|\nadd_spacer|small|\nadd_textbox|This machine is out of order.|left|\nend_dialog|vending|Close||");
	}
	else if (world->items[x + (y * world->width)].opened && world->items[x + (y * world->width)].vcount < world->items[x + (y * world->width)].vprice) {
...
inline void SendDigiVendDialog(ENetPeer* peer, WorldInfo* world, int x, int y) {
	auto squaresign = x + (y * 100);
	string type = "";
	if (world->items[x + (y * world->width)].rm) {
		type = "\nadd_checkbox|chk_peritem|World Locks per Item|1\nadd_checkbox|chk_perlock|Items per World Lock|0";
...
inline void SendBuyerDigiVendDialog(ENetPeer* peer, WorldInfo* world, int x, int y) {
	auto squaresign = x + (y * 100);
	auto currentworld = ((PlayerInfo*)(peer->data))->currentWorld + "X" + std::to_string(squaresign);
	if (world->items[x + (y * world->width)].vid == 0 || world->items[x + (y * world->width)].vcount == 0 || world->items[x + (y * world->width)].vprice == 0) {
		Player::OnDialogRequest(peer, "set_default_color|`o\nadd_label_with_icon|big|`wDigiVend Machine``|left|9268|\nadd_spacer|small|\nadd_textbox|This machine is out of order.|left|\nend_dialog|vending|Close||");
...
inline void UpdateVend(ENetPeer* peer, int blockid, int x, int y, int id, bool locks, int price, int background, bool isPer = false) {
	auto options321 = 0;
...
inline void restore_prices() {
	try {
		char oldname[] = "etc/price/prdata.zep";
		char newname[] = "etc/price/prdata1.zep";
		char oldname1[] = "etc/price/prdata1.zep";
		char newname1[] = "etc/price/prdata2.zep";
		char oldname2[] = "etc/price/prdata2.zep";
		char newname2[] = "etc/price/prdata3.zep";
		auto itemid = 0;
		auto kaina = 0;
		string append = "";
		int drop_k = 0;
...
inline void SendPunishView(ENetPeer* peer, string PlayerName) {
	...
	std::ifstream ifs7("save/gemdb/_" + PlayerName + ".zep");
	...
	if (static_cast<PlayerInfo*>(peer->data)->Fishing && static_cast<PlayerInfo*>(peer->data)->TriggerFish) {
		...
		if (static_cast<PlayerInfo*>(peer->data)->TriggerFish) {
			...
			if (static_cast<PlayerInfo*>(peer->data)->fishermanlevel >= 2 && rand() % 100 <= chanceofbonus) {
				...
				ifs.close();
				currentGems += Gems;
				ofstream ofs("save/gemdb/_" + static_cast<PlayerInfo*>(peer->data)->rawName + ".zep");
				ofs << currentGems;
				ofs.close();
				SendXP(peer, 5);
				Player::OnSetBux(peer, currentGems, 0);
...
inline void restore_player_name(WorldInfo* world, ENetPeer* peer) {
	try {
		...
		{
			name2 = role_prefix.at(static_cast<PlayerInfo*>(peer->data)->adminLevel) + static_cast<PlayerInfo*>(peer->data)->displayName;
			static_cast<PlayerInfo*>(peer->data)->country = "us";
		}
		if (static_cast<PlayerInfo*>(peer->data)->NickPrefix != "")
		{
			name2 = static_cast<PlayerInfo*>(peer->data)->NickPrefix + ". " + static_cast<PlayerInfo*>(peer->data)->displayName;
		}
...
inline void LoadPlayerData(ENetPeer* peer) {
	...
	if (pData->haveGrowId && pData->rawName != "") {
		try {
			...
			string nick = j["nick"];
			int BillId = j["BillId"];
			int BillPrice = j["BillPrice"];
			bool billboard = j["billboard"];
			bool BillPeerWl = j["BillPeerWl"];
			bool BillWlPeer = j["BillWlPeer"];
			bool invis = j["invis"];
			bool ghost = j["ghost"];
			int skin = j["skin"];
			...
			if (subdate == "\u000e") {
				Player::OnConsoleMessage(peer, "Your subscription time was corrupted, so it was removed to recover message Jesen N#9071 details such as how long was your subscription and in game nickname");
				sendSound(peer, "audio/dialog_confirm.wav");
				pData->subdate = "0";
				subdate = "";
				subtype = "";
			...
			pData->BillId = BillId;
			pData->BillPrice = BillPrice;
			pData->billboard = billboard;
			pData->BillWlPeer = BillWlPeer;
			pData->BillPeerWl = BillPeerWl;
