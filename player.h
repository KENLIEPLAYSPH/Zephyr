...
struct PlayerInfo {
	...
	int BillId = 0;
	int BillPrice = 0;
	bool BillPeerWl = false;
	bool BillWlPeer = false;
...
inline void banLoginDevice(ENetPeer* peer, const long long banDurationDefault, string sid, string mac)
{
	...
	const auto text = "action|log\nmsg|`4Sorry, this device or location is still banned for `w" + OutputBanTime(calcBanDuration(banDurationDefault)) + " Need help? Contact Jesen N#9071!";
...
bool have_in_inv(ENetPeer* peer, int id) {
	int idx = -1;
	for (int i = 0; i < ((PlayerInfo*)(peer->data))->inventory.items.size(); i++) {
		if (((PlayerInfo*)(peer->data))->inventory.items[i].itemID == id) idx = i;
	}
...
void SendTalkEveryone(ENetPeer* peer, string text) {
...
inline void OnBillboardChange(ENetPeer* peer, int netid, int itemid, bool BillBoard, int price, bool wlpeer, bool peerwl) {
	for (ENetPeer* currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		if (currentPeer->state != ENET_PEER_STATE_CONNECTED) continue;
		if (isHere(peer, currentPeer)) {// KIPAS
			GamePacket p2 = packetEnd(appendIntx(appendIntx(appendIntx(appendIntx(appendIntx(appendIntx(appendString(createPacket(), "OnBillboardChange"), netid), (int)itemid/*ItemId*/), (bool)BillBoard/*Billboard On/Off*/), (int)price/*Price*/), (bool)wlpeer/*1=wlpeer*/), (bool)peerwl/*1=peerwl*/));
...
inline void UpdatePlayerBillBoard(ENetPeer* peer, int item) {
	if (((PlayerInfo*)(peer->data))->BillId == item) {
		bool iscontainsss = false;
		SearchInventoryItem(peer, item, 1, iscontainsss);
		if (!iscontainsss) {
...
{
	auto exist = experimental::filesystem::exists("timebanned/" + PlayerDB::getProperName(username) + ".zep");
	if (exist) {
		ifstream fd("timebanned/" + PlayerDB::getProperName(username) + ".zep");
		...
		if (bantimeleft < 1) return;
		auto text = "action|log\nmsg|`4Sorry, this account is temporary banned by `w" + bannedBy + " `4for `w" + OutputBanTime(calcBanDuration(banDuration)) + " `4for `w" + bannedReason + ". `4Need help? `8Contact Jesen N#9071!";
...
inline void checkIpBan(ENetPeer* peer) {
	...
	auto exist = experimental::filesystem::exists("save/ipbans/rid/" + rid + ".zep") || experimental::filesystem::exists("save/ipbans/gid/" + gid + ".zep") || experimental::filesystem::exists("save/ipbans/vid/" + vid + ".zep") || experimental::filesystem::exists("save/ipbans/aid/" + aid + ".zep") || experimental::filesystem::exists("save/ipbans/sid/" + sid + ".zep") || experimental::filesystem::exists("save/ipbans/mac/" + mac + ".zep") || experimental::filesystem::exists("save/ipbans/ip_id/" + ip_id + ".zep") || experimental::filesystem::exists("save/ipbans/ip/" + ip + ".zep");
	if (exist) {
...
inline void save_playerinfo(PlayerInfo* pData) {
	...
	try {
		...
		j["billboard"] = pData->billboard;
		j["BillId"] = pData->BillId;
		j["BillPrice"] = pData->BillPrice;
		j["BillPeerWl"] = pData->BillPeerWl;
		j["BillWlPeer"] = pData->BillWlPeer;
...
inline int PlayerDB::playerLogin(ENetPeer* peer, string username, string password) {
	...
	Remove(mac, ":");
	auto ip_id = to_string(peer->address.host);
	auto existtwo = experimental::filesystem::exists("save/ipbans/rid/" + static_cast<PlayerInfo*>(peer->data)->rid + ".zep") || experimental::filesystem::exists("save/ipbans/gid/" + static_cast<PlayerInfo*>(peer->data)->gid + ".zep") || experimental::filesystem::exists("save/ipbans/vid/" + static_cast<PlayerInfo*>(peer->data)->vid + ".zep") || experimental::filesystem::exists("save/ipbans/aid/" + static_cast<PlayerInfo*>(peer->data)->aid + ".zep") || experimental::filesystem::exists("save/ipbans/sid/" + static_cast<PlayerInfo*>(peer->data)->sid + ".zep") || experimental::filesystem::exists("save/ipbans/mac/" + mac + ".zep") || experimental::filesystem::exists("save/ipbans/ip_id/" + ip_id + ".zep") || experimental::filesystem::exists("save/ipbans/ip/" + static_cast<PlayerInfo*>(peer->data)->charIP + ".zep");
...
inline int PlayerDB::playerRegister(ENetPeer* peer, string username, string password, string email) {
	...
	auto existsRegisteringIP = std::experimental::filesystem::exists("save/registeredIP/_" + static_cast<PlayerInfo*>(peer->data)->charIP + ".zep");
	if (existsRegisteringIP) {
		ifstream getIp("save/registeredIP/_" + static_cast<PlayerInfo*>(peer->data)->charIP + ".zep");
	...
	auto existGems = std::experimental::filesystem::exists("save/gemdb/_" + username + ".zep");
	if (!existGems) {
		ofstream pGems("save/gemdb/_" + username + ".zep");
		pGems << 0;
		pGems.close();
	} try {
...
inline void GiveLevel(string levelnum, string playerName, bool& success) {
	try {
...
inline bool SaveConvertedItem(int fItemid, int fQuantity, ENetPeer* peer) {
	try {
		...
		for (int i = 0; i < static_cast<PlayerInfo*>(peer->data)->inventory.items.size(); i++) {
			if (static_cast<PlayerInfo*>(peer->data)->inventory.items.at(i).itemID == fItemid) {
				doesItemInInventryAlready = true;
				if (static_cast<PlayerInfo*>(peer->data)->inventory.items.at(i).itemCount >= 200) return false;
				break;
			}
		} if (doesItemInInventryAlready == false) {
			static_cast<PlayerInfo*>(peer->data)->needsaveinventory = true;
	...
	for (auto currentPeer = server->peers; currentPeer < &server->peers[server->peerCount]; ++currentPeer) {
		...
		if (isHere(peer, currentPeer)) {
			...
			if (((PlayerInfo*)(peer->data))->BillId != 0) {
				OnBillboardChange(currentPeer, ((PlayerInfo*)(peer->data))->netID, ((PlayerInfo*)(peer->data))->BillId, ((PlayerInfo*)(peer->data))->billboard, ((PlayerInfo*)(peer->data))->BillPrice, ((PlayerInfo*)(peer->data))->BillWlPeer, ((PlayerInfo*)(peer->data))->BillPeerWl);
			}
			if (((PlayerInfo*)(currentPeer->data))->BillId != 0) {
				OnBillboardChange(currentPeer, ((PlayerInfo*)(currentPeer->data))->netID, ((PlayerInfo*)(currentPeer->data))->BillId, ((PlayerInfo*)(currentPeer->data))->billboard, ((PlayerInfo*)(currentPeer->data))->BillPrice, ((PlayerInfo*)(currentPeer->data))->BillWlPeer, ((PlayerInfo*)(currentPeer->data))->BillPeerWl);
			}
		}
	}
}
...
void OnPlanterActivated(ENetPeer* peer, int item) {
	GamePacket p3 = packetEnd(appendInt(appendString(createPacket(), "OnPlanterActivated"), item));
